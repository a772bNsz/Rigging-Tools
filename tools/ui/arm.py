...
class MyWindow(QWidget):
    ...
    def rig_hand(self, side):
        getattr(self, side + "_arm").ik_hand()
        pm.select(cl=1)
        return

    def setup_hand(self, side):
        if self.locators:
            new_locators = getattr(self, side + "_arm").ik_hand_setup()
        ...
        self.locators = getattr(self, side + "_arm").ik_hand_setup()
    ...
    def rig_shoulder(self, side):
        ...
        root = pm.ls(sl=1)[0]
        root_con = controls[-1]
        shoulder = Rig(root, side=side, root_control=root_con)
        shoulder.ik()
    ...
    def rig_arm(self, side):
        ...
        root = pm.ls(sl=1)[0]
        root_con = controls[-1]
        arm = Rig(root, side=side, root_control=root_con)
        arm.twist()
        arm.ikfk_switch()
        arm.fk()
        arm.ik()
